...
public class SquareArrayAnalyzer {
    ...
    public static Boolean compare(Integer[] inputArray, Integer[] squaredValues) {
        Integer[] inputArraySquared = new Integer[inputArray.length];
        for (int i = 0; i < inputArray.length; i++) {
            Integer integer = inputArray[i];
            inputArraySquared[i] = integer * integer;
        }
        Arrays.sort(inputArraySquared);
        Arrays.sort(squaredValues);

        return Arrays.toString(inputArraySquared).equals(Arrays.toString(squaredValues));
    }
}
